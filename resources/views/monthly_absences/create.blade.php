                ...
                <div class="card-header">
                    <div class="d-flex justify-content-between align-items-center">
                        <h3 class="card-title">
                            ...
                            @if ($setting)
                                {{ $arabicMonths[$setting->month] ?? 'شهر غير معروف' }} {{ $setting->year }}
                            @endif
                            </h2>

                            @if (isset($setting) && $setting->is_open)
                                <button id="print-absences" class="btn btn-success no-print" title="طباعة">
                                    <i class="fas fa-print ml-1"></i> طباعة الغيابات
                                </button>
                            @endif
                    </div>
                </div>
                <div class="card-body">
                    <div class="alert alert-danger shadow-sm border-0">
                        <h5 class="fw-bold mb-3">
                            <i class="fa-solid fa-triangle-exclamation"></i> هام جداً:
                        </h5>
                        <ul class="mb-0" style="line-height: 1.8;">
                            <li>آخر أجل لحجز الغيابات الشهرية للموظفين هو <strong>15 من كل شهر</strong>، بعد هذا التاريخ لا
                                يمكن تعديل الغيابات الشهرية.</li>
                            <li>يجب طباعة الغيابات الشهرية والتوقيع عليها وختمها من طرف المدير، وتقديمها لمصلحة المستخدمين.
                            </li>
                        </ul>
                    </div>
                    @if (isset($setting) && $setting->is_open)
                        <div class="filter-buttons mt-3 d-flex flex-wrap gap-2">
                            <button
                                class="btn {{ $currentAdm == '' ? 'btn-warning' : 'btn-outline-warning' }} adm-filter-btn"
                                data-adm="">
                                الكل
                    ...
                    <div class="table-responsive print-section">
                        @if (isset($setting) && $setting->is_open)
                            <table class="table key-buttons text-md-nowrap" id="absences-table1">
                                <thead>
                                    <tr>
                                        <th>رمز الموظف</th>
                                        <th>اللقب و الاسم</th>
...
@section('js')
    ...
    <script>
        ...
        const currentMonth = "{{ $setting->month ?? '' }}";
