...
class RwPavarRndmImport implements ToModel 
{
    ...
    public function model(array $row)
{
    try {
        ...
        if (!PaperRndm::where('MATRI', $convertedMatri)->exists()) {
            Log::error("MATRI غير موجود في papers_rndm", ['MATRI' => $convertedMatri, 'row' => $row]);
            return null;
        }

        $migration = RwMigrationRndm::find($this->migrationId);
       

        return new PavarRndm([
